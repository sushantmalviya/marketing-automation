...
export const metaAdsService = {
  ...
  async setActiveAdAccount(account: Pick<MetaAdAccount, "account_id" | "name"> & { currency?: string }) {
    return unwrap<unknown>(await apiClient.post("/api/meta-ads/meta/ad-accounts/select/", account));
  },
  async getCampaigns(accountId: string) {
    return unwrap<MetaCampaign[]>(await apiClient.get("/api/meta-ads/meta/campaigns/list/", { params: { account_id: accountId } }));
  },
  async updateObjectStatus(objectId: string, status: MetaObjectStatus) {
    return unwrap<unknown>(await apiClient.post(`/api/meta-ads/meta/objects/${objectId}/status/`, { status }));
  },
  async updateCampaignStatus(campaignId: string, status: string) {
    return unwrap<unknown>(await apiClient.post(`/api/meta-ads/meta/campaigns/${campaignId}/status/`, { status }));
  },
  async updateCampaign(campaignId: string, payload: { name?: string; status?: string }) {
    return unwrap<unknown>(await apiClient.post(`/api/meta-ads/meta/campaigns/${campaignId}/update/`, payload));
  },
  async deleteCampaign(campaignId: string) {
    return unwrap<unknown>(await apiClient.delete(`/api/meta-ads/meta/campaigns/${campaignId}/delete/`));
  },
  async createCampaign(payload: CreateMetaCampaignPayload) {
    return unwrap<{ campaign_id: string; adset_id: string; ad_id: string; message?: string }>(await apiClient.post("/api/meta-ads/meta/campaigns/create/", payload));
  },
  async updateAdsetBudget(adsetId: string, budget: number) {
    return unwrap<unknown>(await apiClient.post(`/api/meta-ads/meta/adsets/${adsetId}/budget/`, { budget }));
  },
  async getInsights(level: InsightsLevel, id: string, timeRange: InsightsTimeRange = "last_30d", breakdowns?: string[]) {
    ...
    return unwrap<MetaInsights | MetaInsights[]>(await apiClient.get("/api/meta-ads/meta/insights/", { params }));
  },
  async syncHistoricalLeads(formId: string) {
    return unwrap<{ leads: MetaLead[]; synced?: number }>(await apiClient.post(`/api/meta-ads/meta/forms/${formId}/leads/sync/`));
  },
  async getWebhookLeads() {
    return unwrap<{ id: number; email: string; name: string; created_at: string }[]>(await apiClient.get("/api/meta-ads/meta/leads/"));
  },
  async getPixelSettings() {
    return unwrap<{ pixel_id: string; access_token: string; test_event_code: string; is_active: boolean }>(await apiClient.get("/api/meta-ads/meta/pixel/"));
  },
  async savePixelSettings(payload: { pixel_id: string; access_token?: string; test_event_code?: string; is_active: boolean; is_test_action?: boolean }) {
    return unwrap<{ status: string; message: string; test_triggered: boolean }>(await apiClient.post("/api/meta-ads/meta/pixel/", payload));
  },
  async getPixelEventLogs() {
    return unwrap<{ id: string; event_name: string; pixel_id: string; test_event_code: string; status: string; created_at: string }[]>(await apiClient.get("/api/meta-ads/meta/pixel/events/"));
  },
  async uploadMedia(accountId: string, mediaType: "image" | "video", file: File) {
    ...
    return unwrap<{ image_hash?: string; video_id?: string }>(
      await apiClient.post("/api/meta-ads/meta/media/upload/", formData, {
