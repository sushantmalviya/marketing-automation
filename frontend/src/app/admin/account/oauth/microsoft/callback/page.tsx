...
export default function MicrosoftOAuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (error) {
      setStatus("error");
      setErrorMsg(`Microsoft authorization was denied or failed: ${error}`);
      return;
    }

    if (!code) {
      setStatus("error");
      setErrorMsg("No authorization code was provided by Microsoft.");
      return;
    }

    apiClient
      .post("/api/communications/sender-identities/oauth/microsoft/callback/", { code })
      .then(() => {
        setStatus("success");
        setTimeout(() => {
      ...
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-xl dark:border-slate-800 dark:bg-slate-900 text-center">
        {status === "loading" && (
          <div className="space-y-4">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Connecting Microsoft Account...</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Exchanging credentials and verifying Graph API permissions.</p>
          </div>
        )}
        ...
        {status === "success" && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Your Microsoft sender email is ready. Redirecting back to Account Settings...</p>
          </div>
        )}
